...
{
    if (copy_to_user(uarg, &vsd_dev->buf_size, sizeof(*uarg)))
        return -EFAULT;
    return 0;
}
...
// There are some reference to this function in task.txt.
// This code was lost in 2017 version of this course, but appear in 2016.
...
static int vsd_driver_probe(struct platform_device *pdev)
{
    ...
    vsd_dev->buf_size =
        vsd_phy_mem_buf_res->end - vsd_phy_mem_buf_res->start + 1;
